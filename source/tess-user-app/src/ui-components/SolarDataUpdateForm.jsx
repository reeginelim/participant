...
export default function SolarDataUpdateForm(props) {
  const {
    id,
    solarData,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = { ...initialValues, ...solarDataRecord };
    setDevice_id(cleanValues.device_id);
    setInverter(cleanValues.inverter);
    setInverter_model(cleanValues.inverter_model);
    setPv_in(cleanValues.pv_in);
    setPv_model(cleanValues.pv_model);
    setPv_sys(cleanValues.pv_sys);
    setErrors({});
  };
  const [solarDataRecord, setSolarDataRecord] = React.useState(solarData);
  React.useEffect(() => {
    const queryData = async () => {
      const record = id ? await DataStore.query(SolarData, id) : solarData;
      setSolarDataRecord(record);
    };
    queryData();
  }, [id, solarData]);
  React.useEffect(resetStateValues, [solarDataRecord]);
  const validations = {
    device_id: [],
    inverter: [],
    inverter_model: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          await DataStore.save(
            SolarData.copyOf(solarDataRecord, (updated) => {
      ...
      {...rest}
      {...getOverrideProps(overrides, "SolarDataUpdateForm")}
    >
      <TextField
        label="Device id"
        isRequired={false}
