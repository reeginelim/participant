...
export default function EvScheduleUpdateForm(props) {
  const {
    id,
    evSchedule,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = { ...initialValues, ...evScheduleRecord };
    setDevice_id(cleanValues.device_id);
    setErrors({});
  };
  const [evScheduleRecord, setEvScheduleRecord] = React.useState(evSchedule);
  React.useEffect(() => {
    const queryData = async () => {
      const record = id ? await DataStore.query(EvSchedule, id) : evSchedule;
      setEvScheduleRecord(record);
    };
    queryData();
  }, [id, evSchedule]);
  React.useEffect(resetStateValues, [evScheduleRecord]);
  const validations = {
    device_id: [],
  };
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          await DataStore.save(
            EvSchedule.copyOf(evScheduleRecord, (updated) => {
      ...
      {...rest}
      {...getOverrideProps(overrides, "EvScheduleUpdateForm")}
    >
      <TextField
        label="Device id"
        isRequired={false}
